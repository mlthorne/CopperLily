...
export class SmsLookupComponent {
  ...
  lookupMessages(): void {
    ...
    if (!regex.test(this.phoneNumber)) {
      this.errorMessage = "Invalid phone number format. Only the characters '+', '-', '(', ')', and numbers 0-9 are allowed.";
      return;
    }
    ...
    this.http.get<any[]>(`/api/twilio/messages?phoneNumber=${encodeURIComponent(this.phoneNumber)}&limit=${this.messageLimit}`)
      .subscribe(
        ...
        (error) => {
          ...
          const errorDetails = error?.error?.message || error?.message || 'Unknown error occurred';

          // Set the errorMessage with both the static message and error details
          this.errorMessage = `Failed to retrieve messages. Error: ${errorDetails}`;
        }
      );
  }
